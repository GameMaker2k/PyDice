...
def GetDictValueFromDiceList(DiceList, DiceArrayList={}):
    ...
    while (CountNumOfDice < NumOfDice):
        DiceRolls.append(GetDictValueFromDiceNumber(
            DiceList[CountNumOfDice], DiceArrayList)[0])
...
def GetDictValueFromDiceListAlt(DiceList, DiceArrayList=[{}]):
    ...
    while (CountNumOfDice < NumOfDice):
        DiceRolls.append(GetDictValueFromDiceNumber(
            DiceList[CountNumOfDice], DiceArrayList[CountNumOfDice])[0])
        CountNumOfDice = CountNumOfDice + 1
    return DiceRolls
...
def RandomDiceRollAlt(
        MaxNum=6,
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    if (MaxNum > 0):
        DiceRollValue = RandomDiceRoll(
            1, MaxNum, RandType, RandSeed, DiceArray)[0]
    if (MaxNum < 0):
        DiceRollValue = RandomDiceRoll(
            MaxNum, -1, RandType, RandSeed, DiceArray)[0]
    if (MaxNum == 0):
        DiceRollValue = RandomDiceRoll(
            MaxNum, 1, RandType, RandSeed, DiceArray)[0]
    return [DiceRollValue]
...
def RandomMultiDiceRoll(
    ...
    while (CountNumOfDice < NumOfDice):
        DiceRolls.append(
            RandomDiceRoll(
                MinNum[CountNumOfDice],
                MaxNum[CountNumOfDice],
                RandType,
                RandSeed,
                DiceArray)[0])
        CountNumOfDice = CountNumOfDice + 1
...
def RandomMultiDieRoll(
        MinNum=[1],
        MaxNum=[6],
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    DiceRolls = RandomMultiDiceRoll(
        MinNum, MaxNum, RandType, RandSeed, DiceArray)
    return DiceRolls


def RandomMultiDiceRollByXML(
        DiceStrFile,
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    ...
    DiceRolls = RandomMultiDiceRoll(
        dieminv, diemaxv, RandType, RandSeed, DiceArray)
    return DiceRolls


def RandomMultiDieRollByXML(
        DieStrFile,
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    DiceRolls = RandomMultiDiceRollByXML(
        DieStrFile, RandType, RandSeed, DiceArray)
    return DiceRolls


def RandomMultiDiceRollAlt(
        MaxNum=[6],
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    ...
    while (CountNumOfDice < NumOfDice):
        ...
        CountNumOfDice = CountNumOfDice + 1
    DiceRolls = RandomMultiDiceRoll(
        DiceRollsMin, DiceRollsMax, RandType, RandSeed, DiceArray)
    return DiceRolls


def RandomMultiDieRollAlt(
        MaxNum=[6],
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
...
def RandomMultiDiceRollAltByXML(
        DiceStrFile,
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
...
def RandomMultiDieRollAltByXML(
        DieStrFile,
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    DiceRolls = RandomMultiDiceRollAltByXML(
        DieStrFile, RandType, RandSeed, DiceArray)
    return DiceRolls


def RandomMultiSameDiceRoll(
        NumOfDice=1,
        MinNum=1,
        MaxNum=6,
        RandType=1,
...
def RandomMultiSameDieRoll(
        ...
        RandSeed=random.seed(),
        DiceArray=None):
    DiceRolls = RandomMultiSameDiceRoll(
        NumOfDie, MinNum, MaxNum, RandType, RandSeed, DiceArray)
    return DiceRolls


def RandomMultiSameDiceRollAlt(
        NumOfDice=1,
        MaxNum=6,
        RandType=1,
        RandSeed=random.seed(),
...
def RandomMultiSameDieRollAlt(
        NumOfDie=1,
        MaxNum=6,
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    DiceRolls = RandomMultiSameDiceRollAlt(
        NumOfDie, MaxNum, RandType, RandSeed, DiceArray)
    return DiceRolls


def RandomDiceRollByPosition(
        DiceStr="0,0,0,0,0,1",
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    ...
    while (CountNumOfDice < NumOfDice):
        ...
        if (int(DiceStrList[CountNumOfDice]) > 0):
            DiceRollsTmp = RandomMultiSameDiceRollAlt(
                int(DiceStrList[CountNumOfDice]), CurDiceNum, RandType, RandSeed, DiceArray)
            DiceRolls = DiceRolls + DiceRollsTmp
        CountNumOfDice = CountNumOfDice + 1
    return DiceRolls


def RandomDieRollByPosition(
        DieStr="0,0,0,0,0,1",
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
...
def RandomDiceRollByString(
    ...
    DiceRolls = []
    while (CountNumOfDice < NumOfDice):
        GetDiceRoll = int(re.findall(
            "d([0-9]+)", DiceStrList[CountNumOfDice])[0])
        DiceRolls = DiceRolls + \
            RandomDiceRollAlt(GetDiceRoll, RandType, RandSeed, DiceArray)
        CountNumOfDice = CountNumOfDice + 1
    return DiceRolls


def RandomDieRollByString(
        DieStr="d6",
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
...
def RandomMultiDiceRollByString(
        DiceStr="1d6",
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    ...
    DiceRolls = []
    while (CountNumOfDice < NumOfDice):
        GetPreDiceRoll = re.findall(
            "([0-9]*)d([0-9]+)([l|h]?[0-9]*)", DiceStrList[CountNumOfDice])[0]
        if (GetPreDiceRoll[1] == ""):
            GetDiceRoll = 1
        else:
        ...
        if (not GetPreDiceRoll[2] == ""):
            if (len(re.findall(
                    "^([l|h]{1})([0-9]+)$", GetPreDiceRoll[2])) > 0):
                GetDiceAVGList = re.findall(
                    "^([l|h]{1})([0-9]+)$", GetPreDiceRoll[2])[0]
            else:
                GetPreDiceRoll[2] = ""
        GetPreDiceRollList = []
        if (GetNumDice > 0):
            GetPreDiceRollList = GetPreDiceRollList + \
                RandomMultiSameDiceRollAlt(
                    GetNumDice, GetDiceRoll, RandType, RandSeed, DiceArray)
        if (GetPreDiceRoll[2] == ""):
            ...
            if (GetDiceAVGList[0] == "l"):
                GetDiceRollList = GetMinValues(
                    GetPreDiceRollList, int(GetDiceAVGList[1]))
            if (GetDiceAVGList[0] == "h"):
                GetDiceRollList = GetMaxValues(
                    GetPreDiceRollList, int(GetDiceAVGList[1]))
...
def RandomMultiDieRollByString(
        DieStr="1d6",
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    DiceRolls = RandomMultiDiceRollByString(
        DieStr, RandType, RandSeed, DiceArray)
    return DiceRolls
...
def RandomMultiDiceRollByStringAlt(
        DiceStr="d6:1",
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    ...
    while (CountNumOfDice < NumOfDice):
        ...
        if (GetNumDice > 0):
            GetPreDiceRollList = GetPreDiceRollList + \
                RandomMultiSameDiceRollAlt(
                    GetNumDice, GetDiceRoll, RandType, RandSeed, None)
        if (GetPreDiceRoll[2] == ""):
            GetDiceRollList = GetPreDiceRollList
        else:
            if (int(GetDiceAVGList[1]) > int(GetPreDiceRoll[0])):
            ...
            if (GetDiceAVGList[0] == "h"):
                GetDiceRollList = GetMaxValues(
                    GetPreDiceRollList, int(GetDiceAVGList[1]))
        if (DiceArray is not None and type(DiceArray) is dict):
            GetDiceRollList = GetDictValueFromDiceList(
                GetDiceRollList, DiceArray)
        if (DiceArray is not None and (type(DiceArray)
                                       is list or type(DiceArray) is tuple)):
            if (DiceArray[0] is not None and type(DiceArray[0]) is dict):
                GetDiceRollList = GetDictValueFromDiceListAlt(
                    GetDiceRollList, DiceArray)
        DiceRolls = DiceRolls + GetDiceRollList
        CountNumOfDice = CountNumOfDice + 1
    return DiceRolls
...
def RandomMultiDieRollByStringAlt(
        DieStr="d6:1",
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    DiceRolls = RandomMultiDiceRollByStringAlt(
        DieStr, RandType, RandSeed, DiceArray)
    return DiceRolls


def RandomMultiDiceRollByXMLAlt(
        DiceStrFile,
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
...
def RandomMultiDieRollByXMLAlt(
        DieStrFile,
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    DiceRolls = RandomMultiDiceRollByXMLAlt(
        DieStrFile, RandType, RandSeed, DiceArray)
    return DiceRolls


def RandomMultiDiceRollMinMaxByString(
        DiceStr="1d1:6",
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    ...
    DiceStr = re.sub("([0-9]*)u([0-9]+)([l|h]?[0-9]*)",
                     "\\1d-\\2:\\2\\3", DiceStr)
    DiceStrList = DiceStr.split(",")
    NumOfDice = len(DiceStrList)
    CountNumOfDice = 0
    ...
    while (CountNumOfDice < NumOfDice):
        ...
        if (GetNumDice > 0):
            if (GetMaxDiceRoll == ""):
                GetPreDiceRollList = GetPreDiceRollList + \
                    RandomMultiSameDiceRollAlt(
                        GetNumDice, GetMinDiceRoll, RandType, RandSeed, None)
            else:
                GetPreDiceRollList = GetPreDiceRollList + RandomMultiSameDiceRoll(
                    GetNumDice, GetMinDiceRoll, GetMaxDiceRoll, RandType, RandSeed, None)
        if (GetPreDiceRoll[3] == ""):
            GetDiceRollList = GetPreDiceRollList
        else:
            if (int(GetDiceAVGList[1]) > int(GetPreDiceRoll[0])):
...
def RandomMultiDieRollMinMaxByString(
        DieStr="1d1:6",
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    DiceRolls = RandomMultiDiceRollMinMaxByString(
        DieStr, RandType, RandSeed, DiceArray)
    return DiceRolls


def RandomMultiDiceRollMinMaxByXML(
        DiceStrFile,
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    ...
    for child in root:
        ...
        if (child.tag == "coin"):
            ...
            if (len(re.findall("^([0-9]+)$", dienum)) < 1):
                dienum = "1"
            diestr = diestr + dienum + "c"
    DiceRolls = RandomMultiDiceRollNotationMinMaxByString(
        diestr, RandType, RandSeed, DiceArray)
    return DiceRolls


def RandomMultiDieRollMinMaxByXML(
        DieStrFile,
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    DiceRolls = RandomMultiDiceRollMinMaxByXML(
        DieStrFile, RandType, RandSeed, DiceArray)
    return DiceRolls


def RandomMultiDiceRollNotationByString(
        DiceStr="1d6",
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    ...
    while (CountNumOfDice < NumOfDice):
        ...
        if (GetNumDice > 0):
            GetPreDiceRollList = GetPreDiceRollList + \
                RandomMultiSameDiceRollAlt(
                    GetNumDice, GetNewDiceRoll, RandType, RandSeed, None)
        if (GetPreDiceRoll[2] == ""):
            ...
            if (int(GetDiceAVGList[1]) > int(GetPreDiceRoll[0])):
                GetDiceAVGList[1] = GetPreDiceRoll[0]
            if (GetDiceAVGList[0] == "l"):
                GetSubDiceRollList = GetMinValues(
                    GetPreDiceRollList, int(GetDiceAVGList[1]))
            if (GetDiceAVGList[0] == "h"):
                GetSubDiceRollList = GetMaxValues(
                    GetPreDiceRollList, int(GetDiceAVGList[1]))
        ...
        if (not GetPreDiceRoll[4] == ""):
            if (len(re.findall(
                    "^([l|h]{1})([0-9]+)$", GetPreDiceRoll[4])) > 0):
                GetPostDiceAVGList = re.findall(
                    "^([l|h]{1})([0-9]+)$", GetPreDiceRoll[4])[0]
            else:
                GetPreDiceRoll[4] = ""
        if (GetPreDiceRoll[4] == ""):
            ...
            if (GetPostDiceAVGList[0] == "l"):
                GetLastDiceRollList = GetMinValues(
                    GetDiceRollList, int(GetPostDiceAVGList[1]))
            if (GetPostDiceAVGList[0] == "h"):
                GetLastDiceRollList = GetMaxValues(
                    GetDiceRollList, int(GetPostDiceAVGList[1]))
        if (DiceArray is not None and type(DiceArray) is dict):
            GetLastDiceRollList = GetDictValueFromDiceList(
                GetLastDiceRollList, DiceArray)
        if (DiceArray is not None and (type(DiceArray)
                                       is list or type(DiceArray) is tuple)):
            if (DiceArray[0] is not None and type(DiceArray[0]) is dict):
                GetLastDiceRollList = GetDictValueFromDiceListAlt(
                    GetLastDiceRollList, DiceArray)
...
def RandomMultiDieRollNotationByString(
        DieStr="1d6",
        RandType=1,
        RandSeed=random.seed()):
...
def RandomMultiDiceRollNotationByXML(
        DiceStrFile,
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
...
def RandomMultiDieRollNotationByXML(
        DieStrFile,
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    DiceRolls = RandomMultiDiceRollNotationByXML(
        DieStrFile, RandType, RandSeed, DiceArray)
    return DiceRolls


def RandomMultiDiceRollNotationMinMaxByString(
        DiceStr="1d1:6",
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    ...
    while (CountNumOfDice < NumOfDice):
        GetPreDiceRoll = re.findall(
            "([0-9]*)d([\\-]?[0-9]+)([\\:]?[\\-]?[0-9]*)([l|h]?[0-9]*)([\\+\\-\\*\\/]?[0-9\\+\\-\\*\\/]*)([l|h]?[0-9]*)",
            DiceStrList[CountNumOfDice])[0]
        if (GetPreDiceRoll[0] == ""):
            GetNumDice = 1
        else:
...
def RandomMultiDieRollNotationMinMaxByString(
        DieStr="1d1:6",
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
...
def RandomMultiDiceRollNotationMinMaxByXML(
        DiceStrFile,
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
...
def RandomMultiDieRollNotationMinMaxByXML(
        DieStrFile,
        RandType=1,
        RandSeed=random.seed(),
        DiceArray=None):
    DiceRolls = RandomMultiDiceRollNotationMinMaxByXML(
        DieStrFile, RandType, RandSeed, DiceArray)
    return DiceRolls
...
def RandomMultiCoinFlip(
    ...
    while (CountNumOfCoins < NumOfCoins):
        CoinFlipValue = CoinFlipValue + \
            RandomCoinFlip(ReturnValType, RandType, RandSeed)
        CountNumOfCoins = CountNumOfCoins + 1
    return CoinFlipValue


def RandomMultiCoinFlipAlt(
        NumOfCoins=1,
        ReturnValType="int",
        RandType=1,
        RandSeed=random.seed()):
    ...
    while (CountNumOfCoins < NumOfCoins):
        CoinFlipValue = CoinFlipValue + \
            RandomCoinFlipAlt(ReturnValType, RandType, RandSeed)
        CountNumOfCoins = CountNumOfCoins + 1
    return CoinFlipValue
